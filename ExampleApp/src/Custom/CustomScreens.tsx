...
export default function CustomScreens({navigation}: CustomScreensProps) {
  ...
  const editScreen = (item: any, index: number) => {
    navigation.navigate('screenDetails', {screenData: item, isEdit: true, itemIndex: index});
  };
    ...
    return (
      <View style={styles.item}>
        ...
        <View style={styles.buttonCol}>
          <Pressable style={[styles.remove, styles.open]} onPress={() => openScreen(item)}>
            <Text>Open</Text>
          </Pressable>
          <Pressable style={[styles.remove, styles.edit]} onPress={() => editScreen(item, index)}>
            <Text>Edit</Text>
          </Pressable>
          <Pressable style={styles.remove} onPress={() => removeScreenData(index)}>
            <Text>Remove</Text>
          </Pressable>
        </View>
      </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList data={screenList} style={styles.flatlistStyle} keyExtractor={(item) => item.id} renderItem={renderItem} />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  button: {backgroundColor: '#5e74e0', width: 200, height: 40, borderRadius: 25, alignItems: 'center', justifyContent: 'center', alignSelf: 'center', marginTop: 30},
  btnTxt: {color: '#FFFFFF', fontSize: 22, fontWeight: 'bold'},
  headerTxt: {fontSize: 18, marginTop: 25, textAlign: 'center', fontWeight: 'bold'},
  item: {marginTop: 20, paddingLeft: 20, borderRadius: 30, paddingVertical: 10, backgroundColor: '#dbdbdb', flexDirection: 'row', flex: 1},
  itemRow: {flex: 0.9},
  buttonCol: {marginLeft: 5},
  row: {flexDirection: 'row', flex: 1},
  textView: {width: 100},
  itemText: {fontWeight: 'bold', width: 120},
  redText: {color: '#ff0000'},
  blueText: {color: '#00FF'},
  remove: {width: 100, height: 40, borderWidth: 1, borderRadius: 25, justifyContent: 'center', alignItems: 'center', marginLeft: 50, marginTop: 10, backgroundColor: '#ba88b4'},
  open: {backgroundColor: '#66b8f2'},
  edit: {backgroundColor: '#49b528'},
});
