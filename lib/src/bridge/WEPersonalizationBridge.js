...
try {
  ...
  console.error('[WE-Inline-JS] Failed to initialize bridge:', error);
}
...
export const initWePersonalization = () => {
  try {
    ...
    console.error('[WE-Inline-JS] initWePersonalization failed:', error);
  }
};
...
try {
  ...
  console.error('[WE-Inline-JS] Failed to initialize view component:', error);
}

if (!WebengagePersonalizationView) {
  WebengagePersonalizationView = () => {
    console.error('[WE-Inline-JS] View component not found. Please ensure the library is properly linked.');
    return null;
  };
}
